...
contract TeebNFT is ERC721Full {
  ...
  modifier onlyDoctor() {
    require(
      _doctors.has(_msgSender()),
      'DoctorRole: caller does not have the Doctor role'
    );
    _;
  }

  struct Medfile {
    uint256 approved_medics; // Number of doctors approved to modify Medfile
    bytes32 hash_medfile; // Hash of last version of Medfile
  }

  uint256 public total_mf; // Total of medfiles recorded

  mapping(uint256 => Medfile) medfile; // Mapping from token ID to struct Medfile

  constructor() public ERC721Full('TEEB_NFT', 'TEEB') {
   ...
   */
  function newMedfileNFT(address nftOwner, bytes32 hashMF)
    public
    onlyDoctor
    returns (bool)
  {
   ...
   */
  function updateMedfileNFT(uint256 idMF, bytes32 newHash)
    public
    onlyDoctor
    returns (bool)
  {
    require(
      isApprovedForNFT(_msgSender(), idMF),
      'ERC721: doctor is not approved for making changes'
    );
    medfile[idMF].hash_medfile = newHash;
    return true;
  }

  function NFTDescription(uint256 idMF)
    public
    view
    returns (uint256 _approveds, bytes32 _hash, address _owner)
  {
  ...
  function grantAccessTo(address _doctor, uint256 idMF) public returns (bool) {
    require(
      isDoctor(_doctor),
      'DoctorRole: recipient does not have the Doctor Role'
    );
  ...
  function revokeAccessTo(address _doctor, uint256 idMF) public returns (bool) {
    require(
      isApprovedForNFT(_doctor, idMF),
      'DoctorRole: doctor does not have access to NFT Medfile'
    );
    medfile[idMF].approved_medics -= 1;
    revoke(_doctor, idMF);
    return true;
  ...
  function addDoctorRole(address medic) public onlyAdmin returns (bool) {
    // implementar AdminRole
    _doctors.add(medic);
    emit DoctorAdded(medic);
    return true;
  }

  function removeDoctorRole(address medic) public onlyAdmin returns (bool) {
    // implementar AdminRole
  ...
  function addAdminRole(address _admin) public onlyAdmin returns (bool) {
    // implementar AdminRole
    _admins.add(_admin);
  ...
  function removeAdminRole(address _admin) public onlyAdmin returns (bool) {
    // implementar AdminRole
